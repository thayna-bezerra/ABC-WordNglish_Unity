...
public class TutorialController : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (this.gameObject.CompareTag("Tutorial/Movement"))
            {
                tutoMove.gameObject.SetActive(true);
                keys.gameObject.SetActive(true);
            }

            if (this.gameObject.CompareTag("Tutorial/Jump"))
            {
                tutoJump.gameObject.SetActive(true);
                spaceBar.gameObject.SetActive(true);
            }

            if (this.gameObject.CompareTag("Tutorial/Enemy"))
            {
                tutoEnemy.gameObject.SetActive(true);
            }

            if (this.gameObject.CompareTag("Tutorial/Letter"))
            {
